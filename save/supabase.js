// save/supabase.js (Versão Limpa e Organizada)
...
class SupabaseSync {
    ...
    async init() {
        try {
            // Garante que a biblioteca do Supabase foi carregada
            if (typeof supabase === 'undefined') {
                await this.loadSupabaseLibrary();
            }
            const { createClient } = supabase;

            // Conecta ao Supabase usando as chaves do config.js
    ...
    // Carrega a biblioteca do Supabase se ela ainda não existir na página
    loadSupabaseLibrary() {
    ...
    // Monitora se o dispositivo está online ou offline
    setupNetworkMonitoring() {
        window.addEventListener('online', () => { this.isOnline = true; console.log('🌐 Conexão restaurada'); });
        window.addEventListener('offline', () => { this.isOnline = false; console.log('📵 Conexão perdida'); });
    }

    // Garante que a inicialização foi concluída antes de rodar outros comandos
    ...
    // --- MÉTODOS DE AUTENTICAÇÃO CUSTOMIZADA ---

    // Cria o hash da senha da mesma forma que seu sistema original
    ...
    // Cadastra um novo usuário na sua tabela `usuarios`
    ...
    // Loga um usuário verificando na sua tabela `usuarios`
    ...
    // --- MÉTODOS DE DADOS (RICHTEXT E SENTINELA) ---

    // Carrega anotações do RichText
    ...
    // Salva ou atualiza anotações do RichText
    ...
    // Carrega anotações da Sentinela
        ...
        try {
            ...
            if (data?.conteudo_html) {
                ...
                if (wrapper.estudo !== estudoId) {
                    console.warn(`Anotação da nuvem é do estudo ${wrapper.estudo}, mas a página atual é do estudo ${estudoId}`);
                }
    ...
    // Salva ou atualiza anotações da Sentinela
