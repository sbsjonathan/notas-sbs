...
// ====== NOVA FUNÇÃO: ANÁLISE CONTEXTUAL (VERSÃO CORRIGIDA) ======
function analisarContextoRecapitulacao(perguntaEl) {
  ...
  // Lógica geral de concatenação para todos os outros casos (COMO, POR QUE, etc.)
  else {
    const tituloLimpo = tituloCompleto.replace(/\.\.\.$/, '').trim();
    contexto.perguntaCompleta = `${tituloLimpo} ${perguntaIndividual}`;
    contexto.instrucaoFocada = `Responda a pergunta completa e contextualizada: ${contexto.perguntaCompleta}`;
    contexto.explicacao = `Pergunta de recapitulação que requer análise do contexto do título da seção.`;
  }
