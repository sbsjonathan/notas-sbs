      ...
      let frasesValidadas = frases.filter(frase => {
        ...
        if (tipoParte === 'por_que' && perguntaNorm.includes('por que')) {
          return fraseNorm.includes('medo') || fraseNorm.includes('preocupado') || 
                 ...
                 fraseNorm.includes('convide') || fraseNorm.includes('irmãos') ||
                 fraseNorm.includes('diferentes') || fraseNorm.includes('histórias') ||
                 fraseNorm.includes('acompanhar') || fraseNorm.includes('peça') ||
                 fraseNorm.includes('expliquem') || fraseNorm.includes('conheceram') ||
                 fraseNorm.includes('verdade') || fraseNorm.includes('prioridade') ||
                 fraseNorm.includes('bom') || fraseNorm.includes('útil');
        }
  ...
  try {
    ...
    if (isMultiPart) {
      ...
      for (let i = 0; i < partesDetectadas.length; i++) {
        ...
        let frasesValidadas = validarCompletudeEspecifica(
        ...
        // CORREÇÃO ESPECÍFICA: Se a pergunta (a) não retornou frases, força busca manual
        if (parte.letra === 'a' && frasesValidadas.length === 0 && parte.pergunta.toLowerCase().includes('é bom')) {
          const frasesForçadas = [
            /convide\s+irmãos\s+com\s+diferentes\s+histórias[^.]*\./gi,
            /peça\s+que\s+eles\s+expliquem[^.]*\./gi,
            /então,\s+convide\s+irmãos[^.]*\./gi
          ];
          
          frasesForçadas.forEach(regex => {
            const matches = textoLimpo.match(regex);
            if (matches) {
              matches.forEach(match => {
                const fraseLimpa = match.trim().replace(/\s*\([^)]*\)\s*/g, ' ').replace(/\s+/g, ' ');
                if (!frasesValidadas.some(f => f.toLowerCase().includes(fraseLimpa.toLowerCase().substring(0, 20)))) {
                  frasesValidadas.push(fraseLimpa);
                }
              });
            }
