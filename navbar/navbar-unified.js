...
class UnifiedNavbar {
    ...
    getBasePath() {
        ...
        if (path.includes('/save/') || path.includes('/richtext/') || 
            path.includes('/biblia/') || path.includes('/sentinela/')) {
            return '../';
        }
...
async function irParaBiblia(event) {
    ...
    // Se JÁ estiver na seção bíblia → sempre vai para menu (com bypass)
    ...
    // Se vier de FORA da seção bíblia → permite resume automático
    else {
