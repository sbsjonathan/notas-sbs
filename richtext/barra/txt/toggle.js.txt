// toggle.js - Plugin Toggle (sub-slot direito)

class TogglePlugin {
  constructor() {
    this.name = 'toggle';
    this.slotId = '3-right'; // NOVO: Usa sub-slot direito
    this.MAX_LEVEL = 3;
    this.editor = null;
    this.toggleBtn = null;
    this._retryMs = 100;
  ...
  waitForSlotAndRegister() {
    // NOVO: Verifica sub-slot ao invés de slot normal
  ...
  connectToEditor() {
    ...
    console.log('🔗 Plugin Toggle conectado no sub-slot direito');
  }
  ...
  focusElement(element) {
    ...
    // SEMPRE usamos o setTimeout para garantir que a manipulação do cursor
    // aconteça depois que o navegador estiver pronto. Isso resolve o problema
    // da chamada dentro do autoCapitalize.
    setTimeout(() => {
      element.focus();
      const range = document.createRange();
      const sel = window.getSelection();
